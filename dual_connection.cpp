...
namespace mutils{
		...
		std::size_t ControlChannel::raw_receive(std::size_t how_many, std::size_t const * const sizes, void ** bufs){
			if (!consumed_first_byte){
				...
				new_bufs[0] +=1;
				return parent.control.raw_receive(how_many,new_sizes,new_bufs);
			}
			else return parent.control.raw_receive(how_many,sizes,bufs);
		}

		std::size_t ControlChannel::raw_send(std::size_t how_many, std::size_t const * const sizes, void const * const * const bufs){
			return parent.control.raw_send(how_many,sizes,bufs);
		}
		
		bool ControlChannel::valid () const {return parent.control.valid();}
		...
		ControlChannel::~ControlChannel(){
			parent.control_exn_thrown = false;
			assert(parent.check_control_exn.all_idle());
			parent.exn_first_byte = parent.check_control_exn.push(parent.check_control_fun);
		}
		...
		dual_connection::dual_connection(std::unique_ptr<connection> data, std::unique_ptr<connection> control)
			:i(std::move(data),
				...
				[&](int) -> char { try {
					...
					constexpr size_t sizes[] = {&how_many};
					constexpr void* bufs[] = {&first_byte};
					control.raw_receive(how_many,sizes,bufs);
	...
	dual_connection::dual_connection(dual_connection&& o)
		...
		std::size_t dual_connection::raw_receive(std::size_t how_many, std::size_t const * const sizes, void ** bufs){
			if (control_exn_thrown){
				assert(exn_first_byte.ready());
				char first_byte = exn_first_byte.get();
			...
			else return data.raw_receive(how_many,sizes,bufs);
		}
}
