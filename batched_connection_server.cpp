...
namespace mutils{
	namespace batched_connection {
		...
		struct receiver::connection: public ::mutils::connection {
			...
			connection(Socket &s, id_type id whendebug(, std::ofstream& log_file)):s(s),id(id) whendebug(,log_file(log_file)){}
			...
			try { 
				while (alive) {
					...
					auto move_on_receive = [&](){
						...
						whendebug(auto size_rcvd = ) s.receive(size,recv_buf.data());
#ifndef NDEBUG
					...
					tp.push(
						[recv_buf = move_on_receive(),
						 conn = connection{s,id whendebug(, log_file)},
